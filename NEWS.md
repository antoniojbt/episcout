...
- Added an optional `sf`-based vector-mapping foundation through `epi_geo_read()`, `epi_geo_from_coords()`, `epi_geo_describe()`, `epi_geo_transform()`, `epi_geo_map()` and `epi_geo_write()`. The new offline guide covers reviewed coordinate meaning, GeoPackage/Shapefile boundaries, aggregate geometry QA, true CRS transformation, extensible static maps and staged exact-layer GeoPackage publication; feature geometry remains outside ordinary EDA bundles, and PostGIS, inference, raster, web maps and coordinate inference remain deferred.
