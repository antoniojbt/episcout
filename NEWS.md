...
- Added `epi_eda_profile_geo()` and optional reviewed `geo_role`, `geo_pair` and `geo_crs` specification fields. Data-frame and PostgreSQL EDA now publish aggregate coordinate-pair eligibility QA without returning coordinate values, geometry, bounds or maps; coordinate-role variables are excluded from ordinary summaries and plots, and PostgreSQL uses ordinary numeric aggregates without requiring PostGIS.
...
- Added an optional `sf`-based vector-mapping foundation through `epi_geo_read()`, `epi_geo_from_coords()`, `epi_geo_describe()`, `epi_geo_transform()`, `epi_geo_map()` and `epi_geo_write()`. The offline guide covers reviewed coordinate meaning, GeoPackage/Shapefile boundaries, aggregate geometry QA, true CRS transformation, extensible static maps and staged exact-layer GeoPackage publication; feature geometry remains outside ordinary EDA bundles, and inference, raster, web maps and coordinate inference remain deferred.
