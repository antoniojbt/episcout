...
- Added an in-memory probabilistic record-linkage workflow through `epi_linkage_spec()`, `epi_linkage_prepare()`, `epi_linkage_candidates()`, `epi_linkage_compare()`, `epi_linkage_score()`, `epi_linkage_classify()`, `epi_linkage_run()` and `epi_linkage_validate()`. It uses explicit derived representations, exact bounded blocking, declared field comparisons and caller-supplied Fellegi--Sunter parameters and three-way thresholds; complete Cartesian truth keeps blocking recall separate from final decision performance. Model posteriors are not empirically calibrated, routine methods are aggregate-only, and the workflow never writes a crosswalk or identity registry.
- Added `epi_eda_longitudinal_transitions()` for adjacent state-to-state summaries of explicitly selected categorical or binary variables among retained distinct entities. Entity-period missingness and conflicts are classified and reconciled inside PostgreSQL, transition matrices include the complete bounded declared and unexpected state domain, and exact aggregate counts use explicit eligible denominators in one read-only repeatable-read snapshot. Entity values and row-level states are not returned, and the interface makes no scientific or project-specific interpretation.
- Added `epi_eda_longitudinal_drift()` for aggregate-only descriptive comparison of reviewed variables across caller-ordered PostgreSQL periods. It reports schema continuity, missingness and canonical numeric, categorical and temporal summaries in one read-only repeatable-read snapshot; text fields retain schema and missingness evidence with explicit unsupported-distribution evidence. Categorical domains are bounded by `max_levels` (default `50L`) across declarations, periods and adjacent unions and fail rather than truncate when the bound is exceeded. The function returns no source rows or identifiers, does not infer thresholds or anomaly labels, and does not clean data or alter specifications.
...
- Added exact `fill_values` mappings for categorical bars and grouped box plots, with explicit category correspondence, factor-level ordering and validation that rejects incomplete, extra, duplicated or conflicting mappings. The legacy positional `custom_palette` route remains temporarily available with a deprecation warning.
- Added optional creation-time `plot_style` callbacks to in-memory and PostgreSQL EDA plot creation. Callbacks receive completed plots and compact aggregate context rather than source rows or database capabilities; persistent database bundles require a caller-supplied non-secret `plot_style_id` for overwrite provenance.
- Added PostgreSQL-native grouped descriptive summaries through the unchanged `epi_eda_profile_stratified()` interface and opt-in `epi_eda_db_run(strata = ...)` publication. The released stratified result works directly with Table 1 and categorical display, preserves declared empty and missing groups, and uses aggregate-only queries; Shapiro-Wilk is explicitly unavailable because it would require collecting an analysis-value vector.

The immutable release-0.4.1 tag retained `Version: 0.4.0` in `DESCRIPTION`. Published history is preserved rather than retagged.
...
- Added a runnable database-to-report walkthrough with a neutral longitudinal CSV fixture. The commented R script demonstrates duplicate review, PostgreSQL inventory and dictionaries, pseudonymisation through one persisted registry, aggregate-only database EDA, plots, Table 1 and HTML report output without embedding credentials. Stable assignments across later runs require reuse of that same registry and compatible identity mapping.
...
- Added a generic, audit-first PostgreSQL longitudinal pseudonymisation workflow with value-free linkage metadata, a persisted restricted identity registry, exact reviewed crosswalks, explicit longitudinal duplicate handling, atomic output writes and a dedicated first-time-user guide. Recurring identities retain stable assignments only when later runs reuse the same registry and compatible identity mapping. Pseudonymised outputs remain restricted personal data and are not anonymous or automatically disclosure-controlled.
