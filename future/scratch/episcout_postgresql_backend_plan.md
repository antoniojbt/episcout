...
Incorporated into completed spec `003-large-data-backend-strategy` on 2026-08-04 and superseded by its accepted contract and evidence. This file remains historical design input. The accepted performance gate is the neutral fixed synthetic PostgreSQL fixture recorded in the completed spec; no project-specific data, schemas, credentials, dictionaries, fixtures or output conventions belong in `episcout`.
...
The accepted record is `future/specs/done/003-large-data-backend-strategy`. Its reviewed first implementation is PostgreSQL-only; Arrow, DuckDB, data.table, and generic DBI support remain outside this version.
...
Use sequential client execution, PostgreSQL's query planner and server-side parallelism, with deterministic specification order for queries and outputs. Failure of the accepted synthetic gate requires investigation and owner review; it does not authorise multi-connection execution or alternate results.
...
Reconcile selected SVG bins, counts, labels, and exclusions against the fixture and inspect the rendered files. Verify that neither returned objects nor written artifacts contain row-level or identifier values.

Before delivery, run focused tests, the complete package suite, documentation generation, lint, local package checks, CRAN-style checks and the accepted fixed synthetic PostgreSQL gate using the repository's documented entry points.
