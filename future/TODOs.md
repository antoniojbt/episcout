...
Active `issue-285`/`spec-037-epi-sec-linkage-results-neutral` starts from exact canonical closeout `commit-6ba8f25` after `issue-284` completed through final `PR-289` and closeout `PR-290`. This bounded slice replaces confirmation/privacy linkage metadata with explicit output actions, neutralises pseudonymisation results/issues and returns caller-selected ordinary diagnostic values while preserving exact linkage, duplicate, token, destination, transaction, locking, rollback, reconciliation and sanitised database-failure behaviour. Documentation [issue-269](https://github.com/antoniojbt/episcout/issues/269) remains downstream. CodeFactor is informational for low-risk findings; substantive package, PostgreSQL, workflow and review evidence remains required. Roadmap [issue-249](https://github.com/antoniojbt/episcout/issues/249) remains the return point for the existing EDA/deferred queue.
