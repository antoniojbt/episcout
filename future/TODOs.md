...
Issue [#253](https://github.com/antoniojbt/episcout/issues/253) is draft implementation spec `033-categorical-denominator-presentation`. The planning contract defines aggregate-only compatibility, column, row and overall bases; preserves default Table 1 and plotting behaviour; enriches frequency companions and report families without new PostgreSQL queries; and requires a green planning PR before package changes.
