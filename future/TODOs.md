...
`issue-271`/`spec-039-reviewable-qc-cleaning-proposals` and `issue-272`/`spec-040-approved-cleaning-rules-and-processed-outputs` are complete through PR-299 and PR-301, merged to canonical `master` as `commit-78a4776` and `commit-6713a79`. `issue-273`/`spec-041-reviewed-civil-date-derivation` is implemented and locally verified on its task branch with explicit civil-date semantics, exact-midnight validation, immutable sources and new-only data-frame/file/PostgreSQL outputs. Publication, merge and canonical closeout remain pending. This is the terminal authorised cleaning-lane slice and has no automatic successor.
