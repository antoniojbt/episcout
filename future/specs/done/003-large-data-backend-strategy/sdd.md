...
The acceptance workload is a deterministic neutral PostgreSQL fixture created at test runtime with `generate_series()`. It contains 1,000,000 rows and eight reviewed variables: unique bigint identifier, numeric, repeating bigint, 20-level categorical, binary, bounded text length, date and `timestamptz`. Fixed modular rules introduce missingness. Fixture construction and `ANALYZE` are outside the measured interval.

One unmeasured run warms PostgreSQL and creates the first bundle. Three measured calls then use `overwrite = TRUE` against the same unchanged relation and specification. Each interval covers the complete public `epi_eda_db_run()` call, including validation, database queries, compact plot preparation, SVG rendering, serialization, checksum reconciliation, prior-bundle ownership validation and publication. M-005 passes when every run completes and reconciles, non-categorical client fetches remain at most 30 rows, categorical frequency returns 20 rows for `treatment` and two for `flag`, and the median wall-clock time is less than 120 seconds.

The benchmark runs only in the dedicated uninstrumented PostgreSQL integration job, gated by both `EPISCOUT_TEST_POSTGRES=1` and `EPISCOUT_BENCHMARK_POSTGRES=1`; coverage runs do not set the benchmark gate. The threshold has substantial headroom over the recorded planning baseline and is a regression ceiling, not a comparative data-frame benchmark or a production service-level claim.

The data-locality justification is structural: an in-memory data-frame workflow necessarily materialises every selected source row in the R process, so client memory and disclosure exposure scale with row count and may conflict with source-governance rules. The PostgreSQL path instead enforces aggregate query shapes and explicit client-row ceilings, subject to the documented complete categorical-frequency exception. No restricted source or comparative speed claim is needed to establish that boundary.

Failure of the 120-second threshold does not authorise a higher limit, concurrency, approximation or sampling. Investigate the fixed sequential queries, runner health and reproducibility first, then stop for owner review if the gate cannot be met without changing the reviewed contract.
