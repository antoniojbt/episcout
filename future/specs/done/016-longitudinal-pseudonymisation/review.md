...
The CI coverage workflow was extended with the same neutral PostgreSQL 17 service so that live paths are measured rather than skipped. Codecov passed with 92.05% patch coverage against a 90.97% target and 91.21% project coverage, an increase of 0.24 percentage points from `master`. The live suite establishes the required safety boundaries but does not exhaustively fault-inject every PostgreSQL write statement or prove behaviour under every server, driver, backup or administrator logging configuration; those infrastructure controls remain explicitly outside episcout's claims.
