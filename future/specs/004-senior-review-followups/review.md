...
- Added tests that use hand-computed expectations instead of implementation-copy fixture calculations.
- Added descriptive schema tests for integer/numeric and logical/binary current behavior.
- Added explicit optional dependency guards in parallel plotting functions.
- Deferred tidy-eval re-export cleanup as planned.
- Full test suite passed. `devtools::check(manual = FALSE)` passed with one existing NOTE about `.gitkeep` files in `inst/project-template/`.
