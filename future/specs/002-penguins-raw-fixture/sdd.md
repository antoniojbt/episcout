...
After serialization, the generator must read the CSV fixture back with the same
options used by routine tests before deriving expected outputs. Expected schema
types must independently apply the documented EDA observed-type contract rather
than copy package implementation code.
...
Add `tests/testthat/test-penguins-raw-fixtures.R`. It must load and compare the
committed fixture against every `expected_*.csv` file. Plot inventory checks
must inspect non-visual plot structure so they distinguish numeric from
categorical/binary dispatch.
