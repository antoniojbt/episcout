...
Issue [#243](https://github.com/antoniojbt/episcout/issues/243) is now the planning target for draft spec `030-simplify-core-eda-controls-geo-outputs`. Package changes remain gated until its planning/prerequisite PR is green; after that gate, implementation may continue on a stacked draft PR under the owner's explicit instruction.
