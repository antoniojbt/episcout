...
test_that("check_schema matches independently computed expected schema", {
  data <- read.csv(data_path, check.names = FALSE)
  spec <- eda_spec(spec_path)
  expected <- read.csv(expected_schema_path, stringsAsFactors = FALSE)

  observed <- check_schema(data, spec)

  expect_equal(as.data.frame(observed), expected, ignore_attr = TRUE)
})

test_that("check_schema flags missing expected variables", {
  data <- read.csv(data_path, check.names = FALSE)
  spec <- eda_spec(spec_path)
  data$Age <- NULL
...
test_that("check_schema flags unexpected observed variables", {
  data <- read.csv(data_path, check.names = FALSE)
  spec <- eda_spec(spec_path)
