...
The branch introduced a specification-first EDA workflow for `episcout` while preserving the existing lower-level `epi_*` helper API.
...
- variable summaries and plot dispatch through `epi_eda_profile_summaries()` and `epi_eda_profile_plots()`;
...
Expected fixture outputs were intended to be independently computed, not generated from the implementation under test.

## Current Status

The package-facing workflow is now described in `README.md` and the package vignette. The archived planning files remain as rationale and review history.
...
The MVP intentionally keeps summaries and plots basic. Synthetic data are for pipeline preparation and testing only. Large-data backends such as Arrow, DuckDB and data.table remain future work.
