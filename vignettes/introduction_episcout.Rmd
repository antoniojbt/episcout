...
Below are a number of examples of how to use lower-level episcout helper functions. For a complete specification-first EDA workflow, including lean semantic dictionaries and opt-in bounded point maps, see the `specification-first-eda` vignette. episcout creates the EDA outputs explicitly requested by the analyst and does not decide whether they may be shared. For stable pseudonyms across related PostgreSQL tables, see the friendly, audit-first `longitudinal-pseudonymisation` vignette before handling restricted data. To practise the complete journey with a neutral CSV and disposable PostgreSQL database, open the installed `examples/db-to-report/walkthrough.R` script.
...
- specification-first EDA workflows: epi\_eda\_\*
...
```{r}
epi_clean_get_dups(df, "var_id", 1)
```
