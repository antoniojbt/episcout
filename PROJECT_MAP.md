...
- Completed `issue-271`/`spec-039-reviewable-qc-cleaning-proposals` records planning PR-298 and implementation PR-299, merged to canonical `master` as `commit-9f48a06` and `commit-78a4776`. Active `issue-272`/`spec-040-approved-cleaning-rules-and-processed-outputs` implements the distinct executable schema, complete in-memory/CSV/RDS outputs and new-only transactional PostgreSQL materialisation with aggregate reconciliation. `issue-273` remains staged behind issue-272.
