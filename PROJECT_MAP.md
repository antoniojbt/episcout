...
- Completed design-only spec `026-epi-geo-series-plan` is preserved under `future/specs/done/`; issue #226 is active under spec `027-epi-geo-phase-a` for the file/in-memory implementation. Read-only PostGIS successor issue #233 is blocked until Phase A merge/closeout, while aggregate EDA-coordinate integration remains deferred in the design record.
