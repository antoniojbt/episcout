...
- Completed `issue-271`/`spec-039-reviewable-qc-cleaning-proposals` records planning PR-298 and implementation PR-299, merged to canonical `master` as `commit-9f48a06` and `commit-78a4776`. The read-only API returns separate opaque-keyed aggregate evidence and pending proposals for data-frame and PostgreSQL sources without changing semantic metadata or approving rules. `issue-272` is the next implementation slice; `issue-273` remains staged behind it.
