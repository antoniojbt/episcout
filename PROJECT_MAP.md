...
- Issue-358/spec-054 is an active design-only audit of generic transversal association diagnostics currently duplicated in Oferta. Current evidence reuses `epi_eda_profile_stratified()` for categorical cross-tabs, retains every project selection and plot downstream, and identifies a bounded successor candidate for explicit-pair Spearman, aggregate-count Cramér's V and fail-not-truncate stratified domains. No package or Oferta behaviour changes under issue #358.
