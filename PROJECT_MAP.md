...
- Completed `issue-271`/`spec-039-reviewable-qc-cleaning-proposals` and `issue-272`/`spec-040-approved-cleaning-rules-and-processed-outputs` record the separate proposal and approved-rule contracts through PR-299 and PR-301, merged as `commit-78a4776` and `commit-6713a79`. `issue-273` is the terminal civil-date successor after canonical closeout.
