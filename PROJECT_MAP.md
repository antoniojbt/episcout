...
- Completed issue-323/PR-324 replaced the ambiguous EDA specification field `type` with required `database_type` and `analysis_type`, merged to canonical `master` as `commit-915b86d` and closed out through PR-326 at `commit-7ac8ea1`. The implementation had no numbered specification. Issue-327/spec-047 is in review through PR-328 as the documentation and version-alignment successor and changes no exported behaviour.
