...
- Completed issue-358/spec-054/PR-367 audited generic transversal association diagnostics currently duplicated in Oferta and merged to canonical `master` as `commit-3d8bbdb`. The accepted boundary reuses `epi_eda_profile_stratified()` for categorical cross-tabs, retains every project selection and plot downstream, and records separately blocked successor #369 for explicit-pair Spearman, aggregate-count Cramér's V and fail-not-truncate stratified domains. Issue #358 changed no package or Oferta behaviour.
