...
- Draft `issue-271`/`spec-039-reviewable-qc-cleaning-proposals` defines the next read-only aggregate QC evidence and pending-proposal contract. Package implementation remains gated on planning review; staged `issue-272` and `issue-273` retain approved-rule application and civil-date derivation respectively.
