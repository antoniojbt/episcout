...
- Completed design spec `026-epi-geo-series-plan` and implementation spec `027-epi-geo-phase-a` are preserved under `future/specs/done/`; issue #226 closed through PR #234, merged as `b37b391`. Read-only PostGIS issue #233 is active as spec `028-epi-geo-postgis` on a branch stacked on green closeout PR #236 under the owner's activation exception, while aggregate EDA-coordinate integration remains gated in successor issue #237.
