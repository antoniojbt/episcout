...
- Active `issue-271`/`spec-039-reviewable-qc-cleaning-proposals` implements the accepted read-only aggregate QC evidence and pending-proposal contract after planning PR-298 merged at canonical commit-9f48a06; staged `issue-272` and `issue-273` retain approved-rule application and civil-date derivation respectively.
