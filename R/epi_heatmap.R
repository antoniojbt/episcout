###################### Helper functions for plotting a heatmap Antonio
###################### Berlanga-Taylor

###################### Plot correlations between numeric variables Set up
###################### correlation matrix with p-values:

##### Get a correlation matrix using Hmisc for many
...
epi_heatmap_corr <- function(df, method = "spearman") {
    ...
    cormat_all <- list(cormat = cormat, cormat_melted_r = cormat_melted_r, 
        cormat_melted_pval = cormat_melted_pval)
    return(cormat_all)
}
# Test data: TO DO: add actual test data df <-
# input_data[, vars_list] cormat_all <-
# epi_heatmap_corr(df) names(cormat_all)
# cormat_all$cormat$n cormat_all$cormat_melted_r
# class(cormat_all)

##### Keep only the lower triangle of the correlation
##### matrix for a nicer heatmap Requires the original,
##### unmelted correlation matrix with both correlation
...
epi_heatmap_corr_triangle <- function(cormat_all) {
    ...
    # Melt and remove NAs:
    cormat_melted_triangle_r <- melt(cormat_tri$cormat$r, 
        na.rm = TRUE)
    # And the same for p-values:
    cormat_tri$cormat$P[lower.tri(cormat_tri$cormat$P)] <- NA
    # Melt:
    cormat_melted_triangle_pval <- melt(cormat_tri$cormat$P, 
        na.rm = TRUE)
...
# Test: melted_triangles <-
# epi_heatmap_corr_triangle(cormat_all)
# melted_triangles$cormat_melted_triangle_r
# melted_triangles$cormat_melted_triangle_pval
# class(melted_triangles)

##### Rename variables from melted triangles and convert
##### to factors for plotting: Requires correlation
##### matrices with r and p-values with columns named
##### Var1 and Var2 The outputs of
##### epi_heatmap_corr_triangle() can be passed here
##### vars_list and var_labels must match by position
##### Variables are converted to factors and re-labelled
##### Values are rounded to digits (default is 2 so that
##### they fit in the heatmap)
epi_heatmap_rename <- function(r_vals = "melted_triangles$cormat_melted_triangle_r", 
    ...
    r_vals$Var1 <- factor(r_vals$Var1, levels = vars_list, 
        labels = var_labels)
    r_vals$Var2 <- factor(r_vals$Var2, levels = vars_list, 
        labels = var_labels)
    # head_and_tail(cormat_melted_triangle_r, cols = 3)
    # summary(cormat_melted_triangle_r) Rename p-values:
    p_vals$Var1 <- factor(p_vals$Var1, levels = vars_list, 
        labels = var_labels)
    p_vals$Var2 <- factor(p_vals$Var2, levels = vars_list, 
        labels = var_labels)
    # head_and_tail(cormat_melted_triangle_pval, cols =
    # 3) summary(cormat_melted_triangle_pval) # Add
    # rounded correlation coefficients to plot:
...
# Test: TO DO: add vars_list and var_labels
# renamed_triangles <-
# epi_heatmap_rename(melted_triangles$cormat_melted_triangle_r,
# melted_triangles$cormat_melted_triangle_pval,
# vars_list = vars_list, var_labels = var_labels )
...
###################### Heatmap ggplot2 wrapper function for a triangle
###################### heatmap Requires a data.frame with correlations and
###################### a data.frame with matching p-values. Columns
###################### headers in both must be called Var1 and Var2.  Use
###################### hmisc::rcorr(), data.table::melt(),
###################### cormat[lower.tri(cormat)] <- NA to get these
###################### outputs for example.  cor_method is a string passed
...
# ggsave('epi_heatmap_triangle.svg', height = 12,
# width = 12, units = 'in')
