...
#'
#' @description This function takes a dataframe and a lookup dataframe that contains the mapping of factors' levels and labels for specific variables. It applies these levels and labels to the corresponding variables in the target dataframe.
#'
#' @param data_df A dataframe to which factor levels and labels will be applied. This dataframe should contain columns that match the variable names specified in the lookup dataframe.
#' @param lookup_df A lookup dataframe containing at least three columns: `variable`, `level`, and `label`, where `variable` is the name of the column in `data_df` for which levels and labels are specified, `level` are the factor levels (as character), and `label` are the corresponding labels for these levels.
#'
#' @return A modified version of `data_df` with specified columns converted to factors with levels and labels as defined in `lookup_df`.
#'
#' @examples
